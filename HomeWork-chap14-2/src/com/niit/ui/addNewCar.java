...
public class addNewCar extends JFrame{
	...
	public addNewCar( MainFrame mainFrm){
		//初始化数据
	...
	private void initComponent(){
		//初始化标题数组
		lblArray = new String[]{"品牌","车型","排量","里程","价格","离合器","上牌时间"};
		//遍历数组，构建标题
		...
		//设置品牌下拉列表的位置
		cboBrand.setBounds(110, 10, 100, 20);
		//构建车型下拉列表
		cboCarType = new JComboBox();
		ArrayList<CarType> allCarType = cartypeDao.findBrandCarType(1);
		for(int i = 0; i < allCarType.size(); i++){
			cboCarType.addItem(allCarType.get(i).getTypeName());
		}
		cboCarType.setBounds(110, 80, 100, 20);
		//构架文本框
		txtOutput = new JTextField();
		txtMileage = new JTextField();
		txtPrice = new JTextField();
		txtOutput.setBounds(110, 150, 100, 20);
		txtMileage.setBounds(110, 220, 100, 20);
		txtPrice.setBounds(110,290, 100, 20);
		//构建离合器单选按钮
		...
		for(int i = 0 , x = 110; i < rdoClutch.length; i++ , x+=90){
			...
			rdoClutch[i].setBounds(x, 360, 80, 20);
		...
		cboBoardYear.setBounds(110, 430, 70, 20);
		cboBoardMonth.setBounds(190, 430, 70, 20);
		cboBoardDay.setBounds(270, 430, 70, 20);
		//实例化保存 取消按钮
		btnSave = new JButton("保存");
		btnCancel = new JButton("取消");
		btnSave.setBounds(120, 510, 70, 20);
		btnCancel.setBounds(240, 510, 70, 20);
